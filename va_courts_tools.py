...
class VaCourtsTool:
    ...
    def initialize(self):
        ...
        try:
            ...
            if resp.status_code != 200:
                if self._debug: print(f'failed to init the session {resp.status_code}')
            ...
            # if self._debug:
            #     print("Hidden data found: ", self._court_data)

            self._session = sess
            return (True,)
        except Exception as e: return (False, -1, e)
    ...
    def get_cases_for(self, courtIdx: int, date: datetime):
        ...
        try:
            ...
            if resp.status_code != 200:
                if self._debug: print(f'failed to change court ({resp.status_code})')
            ...
            if resp.status_code != 200:
                if self._debug: print(f'failed to extract court data ({resp.status_code})')
                return (False, 'failed to extract court data', resp.status_code)

            if self._debug and len(_cases):
                print(f"{self._date_to_string(case['Hearing Date'])}: court '{case['Court Name']}' data extracted...", len(_cases))
                with open('caught.html', 'wb') as f:
                    f.write(resp.content)
    ...
    def get_all_cases(self, _from: datetime, _to: datetime = None):
        try:
            ...
            _cases = []
            while _from <= _to:
                print(f"# --- extracting data from {self._date_to_string(_from)}...")
                for i in range(self.get_courts_count()):
                    result = self.get_cases_for(i, _from)
                    if result[0]: _cases.extend(result[1])
                _from = _from + timedelta(days=1)
                print(_from, _to, "can move on?", (_from <= _to))

            if self._debug:
                print(f"# --- extracted {len(_cases)} cases from {self._date_to_string(_from)} to {self._date_to_string(_to)}")
            return (True, _cases)
        except:
            if self._debug:
                print('failed to extract cases')
            return (False, 'failed to extract cases')
